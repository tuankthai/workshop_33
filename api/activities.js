...
router.get('/:activityId', async (req, res, next) => {
  try {
    console.log("inside GET /api/activities/:activityId")
    ...
    if (activities) {
      ...
      next({
        name: 'NotFound',
        message: `No Activity ${req.params.activityId}`
      })
    }
  } catch (error) {
    next(error);
...
router.post('/', requireUser, requiredNotSent({requiredParams: ['name', 'description']}), async (req, res, next) => {
  try {
    console.log("inside POST /api/activities ")
...
router.patch('/:activityId', requireUser, requiredNotSent({requiredParams: ['name', 'description'], atLeastOne: true}), async (req, res, next) => {
  try {
    ...
    console.log("inside PATCH /api/activities/:activityId")
    if(!existingActivity) {
      next({
        name: 'NotFound',
